...
export const sentencize = (
  text: string
): Effect.Effect<ReadonlyArray<string>> => runNLP(Effect.flatMap(NLPService, (svc) => svc.sentencize(text)))
...
export const paragraphize = (
  text: string
): Effect.Effect<ReadonlyArray<string>> => runNLP(Effect.flatMap(NLPService, (svc) => svc.paragraphize(text)))
...
export const ngrams = (
  text: string,
  n: number
): Effect.Effect<ReadonlyArray<string>> => runNLP(Effect.flatMap(NLPService, (svc) => svc.ngrams(text, n)))
